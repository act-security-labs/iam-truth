...
function mergedResult(rows: TruthTableScenarioRow[], rowIndexes: number[]): TruthTableRowResult {
  ...
  if (firstResult.resultType === 'explicitlyDenied') {
    const matchedStatements = new Map<number, TruthTableMatchedStatement>()
    for (const index of rowIndexes) {
      const result = rows[index].result
      if (result.resultType === 'explicitlyDenied') {
        for (const statement of result.matchedStatements) {
          if (!matchedStatements.has(statement.index)) {
            matchedStatements.set(statement.index, statement)
          }
        }
      }
    }
    return {
      ...
      matchedStatements: [...matchedStatements.values()].sort(
