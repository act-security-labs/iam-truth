...
import {
  ...
  type TruthTableRequestOverrides
} from './types.js'
...
export interface BaselineRequest {
  ...
  /** Representative IAM resource used for scenario metadata and single-resource simulation calls. */
  resource: string
...
/** Caller-provided request values used when resolving generated simulation baselines. */
export type BaselineRequestOverrides = TruthTableRequestOverrides

/** Result of resolving baseline request defaults and overrides. */
export type ResolveBaselineRequestResult =
  | { resultType: 'success'; request: BaselineRequest; resources: string[] }
...
export async function resolveBaselineRequest(
  ...
  overrides: BaselineRequestOverrides | undefined
  ...
  const resources = normalizedResources(overrides?.resources)
...
/**
 * Normalizes caller-provided resource strings into the resources to evaluate.
 *
 * @param resources - Optional requested resources.
 * @returns A defensive copy of requested resources, or the default resource when none were provided.
 */
function normalizedResources(resources: string[] | undefined): string[] {
