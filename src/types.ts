...
export interface TruthTableRequestOverrides {
  ...
  /** IAM resource string to simulate for each generated scenario. */
  resource?: string
...
export interface TruthTableGenerationOptions {
  /** Maximum scenarios to evaluate before returning a fatal too-many-scenarios result. Defaults to 1000. */
  maxRows?: number

  /** Scenario count that should produce a warning diagnostic while still evaluating. Defaults to 25. */
  warnAtRows?: number
...
/** Result returned when scenario generation exceeds the configured maximum. */
export interface TooManyScenariosTruthTablesResult {
  /** Discriminant for too many generated scenarios. */
  resultType: 'tooManyScenarios'

  /** Number of scenarios that would have been generated. */
  scenarioCount: number

  /** Maximum scenario count allowed by options. */
  maxScenarios: number

  /** Diagnostics explaining the scenario limit. */
  diagnostics: TruthTableDiagnostic[]
}
...
export interface TruthTableScenarioRow {
  /** Optional row discriminant. Omitted for backwards-compatible concrete scenario rows. */
  rowType?: 'scenario'
