...
export function valuesForConditionKey(
  ...
  if (parsedOperator.baseOperator.toLowerCase() === 'null') {
    values.push(...nullOperatorValues(conditionKey))
    return uniqueScenarioValues(values)
  }
...
function multiValueSetOperatorValues(
  ...
  const firstMatching = matchingValues[0] ?? 'cloud-copilot-example-value'
...
function alternateValue(
  ...
  if (metadata.valueType === 'arn') {
    return {
      cellValue: 'arn:aws:iam::222222222222:role/OtherRole',
      contextValue: 'arn:aws:iam::222222222222:role/OtherRole'
    }
  }
  if (operator.includes('like')) {
  ...
  const lowerKey = conditionKey.key.toLowerCase()
  if (lowerKey.includes('orgid')) {
    const value = firstDistinctString(conditionKey.values, ['o-otherorg', 'o-example-alt'])
    return { cellValue: value, contextValue: value }
  }
  if (lowerKey.includes('account')) {
    const value = firstDistinctString(conditionKey.values, ['222222222222', '333333333333'])
    return { cellValue: value, contextValue: value }
  }
  if (lowerKey.includes('orgpaths')) {
    return {
      cellValue: 'o-otherorg/r-root/ou-other/',
      contextValue: ['o-otherorg/r-root/ou-other/']
    }
  }

  const value = firstDistinctString(conditionKey.values, [
    'cloud-copilot-other-value',
    'cloud-copilot-alternate-value'
  ])
  return { cellValue: value, contextValue: value }
}
 ...
 * @param candidates - Preferred alternate values in priority order.
 * @returns The first distinct candidate, or a deterministic suffixed fallback.
 */
function firstDistinctString(existingValues: string[], candidates: string[]): string {
 ...
 */
function nullOperatorValues(conditionKey: ExtractedConditionKey): ScenarioValue[] {
  const wantsMissing = conditionKey.values.some((value) => value.toLowerCase() === 'true')
  const presentValue = {
    cellValue: 'context-key-present',
    contextValue: 'context-key-present'
  }
