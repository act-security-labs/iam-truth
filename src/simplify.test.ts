...
describe('simplifyTruthTable', () => {
  ...
  it('should merge explicit denies with the same user-facing result and combine matched statements', () => {
    ...
    const table = tableWithRows(
      ['a', 'b'],
      [row(1, { a: 'x', b: '1' }, denied('DenyOne')), row(2, { a: 'x', b: '2' }, denied('DenyTwo'))]
    )

    //When the table is simplified
    const result = simplifyTruthTable(table)

    //Then the summary should combine matched statement IDs for traceability
    expect(result.rows).toEqual([
      {
        rowType: 'summary',
        rowId: 'row-1',
        ...
        result: {
          ...
          label: 'Denied',
          matchedStatementIds: ['DenyOne', 'DenyTwo']
        },
        coveredRowCount: 2,
        coveredRowIds: ['row-1', 'row-2']
      }
...
function tableWithRows(conditionKeys: string[], rows: TruthTableScenarioRow[]): PolicyTruthTable {
  const columns: TruthTableColumn[] = [
    ...conditionKeys.map((key) => ({ key, label: key, valueType: 'string' as const })),
 ...
 *
 * @param statementId - Matched statement ID.
 * @returns Explicitly denied row result.
 */
function denied(statementId: string): TruthTableRowResult {
  return { resultType: 'explicitlyDenied', label: 'Denied', matchedStatementIds: [statementId] }
}
